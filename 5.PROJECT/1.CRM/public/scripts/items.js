...
async function getItems({ page }) {
  // const res = await fetch(`/api/items?name=${encodeURIComponent(name)}&gender=${encodeURIComponent(gender)}?page=${page}`);
...
async function getItemCount() {
  // const res = await fetch(`/api/items/count?name=${encodeURIComponent(name)}&gender=${encodeURIComponent(gender)}`);
...
function renderPagination({ page = 1, pageSize, totalCount }) {
  ...
  // const baseURL = `<a href="/items?name=${name}&gender=${gender}`;
...
document.getElementById('searchForm').addEventListener('submit', e => {
  ...
  loadData('`http://127.0.0.1:3000/items?');
  // loadData(`http://127.0.0.1:3000/items?page=${page}`);
});
 */